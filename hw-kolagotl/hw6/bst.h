...
* are freed within the clearHelper() helper method in the BinarySearchTree.
*/
template<typename Key, typename Value>
Node<Key, Value>::~Node()
{
...
protected:
    ...
    Node<Key, Value>* findSuccessor(Node<Key, Value>* begin);
    // add a getHeight function
...
{
    // TODO
    Node<Key, Value>* repeat = NULL;
    // checks if the inserted value is a repeat
    repeat = internalFind(keyValuePair.first);
    ...
    if (root_ == NULL){
        ...
        repeat -> setValue(keyValuePair.second);
        return;
    }
    ...
    // inserts a value
        ...
        while (curr != NULL){
            ...
            if (curr_parent ->getKey() == keyValuePair.first) {
                curr_parent ->setValue(keyValuePair.second);
                return;
            }
...
void BinarySearchTree<Key, Value>::remove(const Key& key)
{
    ...
    Node<Key, Value> *grandR;                         
    Node<Key, Value> *grandL;
    Node<Key, Value> *pred = predecessor(curr);
    Node<Key, Value> *tempLeft;
...
BinarySearchTree<Key, Value>::predecessor(Node<Key, Value>* current)
{
    ...
    if (temp != NULL){
        //for the left node move one to the left
        ...
        //If the successor is a parent traverse until you hit a subtree
        else {
...
template<typename Key, typename Value>
Node<Key, Value>* 
BinarySearchTree<Key, Value>::findSuccessor(Node<Key, Value>* removing)
{
...
BinarySearchTree<Key, Value>::getSmallestNode() const
{
    ...
    //go through the left side of the tree until you hit the bottom most node
...
{
    // TODO
    //checks heights and returns if balanced or not

    if(root_ == NULL) {
        return true;
    }

    int l_h = getHeight(root_->getLeft());
    int r_h = getHeight(root_->getRight());
    int l_b = isBalanced(root_->getLeft());
    int r_b = isBalanced(root_->getRight());
    if(std::abs(l_h - r_h) <= 1 && l_b && r_b) {
        return true;
     }
...
int BinarySearchTree<Key, Value>::getHeight(Node<Key, Value>* root) const
{
    ...
    int l_h = getHeight(root->getLeft());
    int r_h = getHeight(root->getRight());
    int final = std::max(l_h, r_h);
    return final + 1;
